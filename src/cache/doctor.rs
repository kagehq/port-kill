...
pub async fn doctor() -> DoctorReport {
        ...
        for (dir, threshold) in &large_caches {
            ...
            if path.exists() {
                if let Ok(size) = get_dir_size(&path) {
                    if size > *threshold {
                        warnings.push(format!("Large cache detected: {} ({:.1} MB)", 
                            dir, size as f64 / 1_000_000.0));
                    }
                }
            }
        }
    ...
    if backup_dir.exists() {
        if let Ok(entries) = fs::read_dir(&backup_dir) {
            ...
            notes.push(format!("Found {} backup(s) from previous clean operations", count));
        }
    }
    ...
    DoctorReport { ok, notes, warnings, errors }
}
