  ...
  <aside class="w-64 h-screen flex flex-col">
    ...
    <div class="p-4 py-3 border-b border-gray-500/10 flex flex-items-center justify-between">
      ...
      <div class="px-2 py-1.5">
        <div class="flex items-center space-x-2">
          ...
          <span class="text-xs font-medium" :class="isConnected ? 'text-gray-400' : 'text-red-400'">{{ isConnected ? 'Connected' : 'Disconnected' }}</span>
        </div>
      </div>
    </div>
    ...
    <nav class="p-4 flex-1 overflow-y-auto">
      <ul class="space-y-2">
        ...
        <li>
          <NuxtLink to="/processes" class="flex items-center space-x-3 px-3 py-2 text-sm rounded-xl border border-gray-500/10 text-white"
            :class="{ 'bg-gray-500/10 font-medium': route.path.startsWith('/processes'), 'hover:bg-gray-500/10': !route.path.startsWith('/processes') }">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-4 text-gray-500">
        ...
        <li>
          <NuxtLink to="/history" class="flex items-center space-x-3 px-3 py-2 text-sm rounded-xl border border-gray-500/10 text-white"
            :class="{ 'bg-gray-500/10 font-medium': route.path.startsWith('/history'), 'hover:bg-gray-500/10': !route.path.startsWith('/history') }">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-4 text-gray-500">
